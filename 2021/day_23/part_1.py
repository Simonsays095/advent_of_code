...
class Amphipod:
    ...
    def AtDestination(self):
        ...
        # 2) The room below it has a pod in it that's at its destination
    ...
    def GetPodMoves(self, pod: Amphipod, prev_moves: tuple[Move] = tuple()) -> tuple[Set[tuple[Move]], bool]:
        ...
        for neighbor in pod.block.neighbors:
            ...
            if not isinstance(neighbor, FillableBlock):
                # print(f"{neighbor.__class__.__name__} at {neighbor.pos} isn't fillable")
                continue
            # 2) It is fillable but it's not empty,
            if neighbor.occupant is not None:
                # print(f"{neighbor.__class__.__name__} at {neighbor.pos} is already occupied")
                continue
            # 3) pod has already visited neighbor in this string of moves
            if neighbor in [move.block for move in prev_moves]:
                # print(f"We've already been to {neighbor.__class__.__name__} at {neighbor.pos}")
                continue
            # 4) pod started at neighbor
            if neighbor is starting_block:
                # print(f"We started at {neighbor.__class__.__name__} at {neighbor.pos}")
                continue
            ...
            if isinstance(neighbor, Room) and isinstance(pod.block, Hallway):
                ...
                if neighbor.pos[0] != pod.destination:
                    # print(f"Cannot move into Room at {neighbor.pos}, not in the destination column")
                    continue
                # 6b) the neighboring room has a pod that isn't in its destination
                roomNeighbor = next((block for block in neighbor.neighbors if isinstance(block, Room)), None)
                if roomNeighbor.occupant is not None and not roomNeighbor.occupant.AtDestination():
                    # print(f"In a room with an unfinished neighbor next door")
                    continue

            # We can definitely move to this block now. Recursively find later moves
            # print(f"Can move to {neighbor.__class__.__name__} at {neighbor.pos} - looking for later moves")
            curMove = (Move(pod, neighbor),)
            with pod.ApplyMoves(curMove):
            # with pod.AtBlock(neighbor):
