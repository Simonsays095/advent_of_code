...
class Line:
    def __init__(self, x1, y1, x2, y2):
        ...
        if x1 == x2:
            ...
            if y1 < y2:
                # slope = 1
                for offset in range(y2-y1+1):
                    self.points.append([x1+offset,y1+offset])
            else:
                # slope = -1
                for offset in range(y1-y2+1):
                    self.points.append([x1+offset,y1-offset])
        else:
            raise ValueError("Only vertical, horizontal, and diagonal lines allowed")
