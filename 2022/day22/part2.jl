...
function squareup(itr)
    return collect(Iterators.partition(itr, 50))
end
...
function main(loglevel)
    ...
    lines = [rpad(line, maxlen) for line in lines]
    cubemap = reduce(vcat,permutedims.(collect.(lines)))
    BOX_SIZE = 50
    startshape = size(cubemap)
    endshape = (div(size(cubemap, 1), BOX_SIZE), div(size(cubemap, 2), BOX_SIZE))
    # cubemap = squareup(squareup.(lines))
    # cubemap = reshape(cubemap, (div(startshape[1],BOX_SIZE), BOX_SIZE, div(startshape[2],BOX_SIZE), BOX_SIZE))
    cubemap = [[cubemap[BOX_SIZE*(i-1)+1:BOX_SIZE*i,BOX_SIZE*(j-1)+1:BOX_SIZE*j] for j in Base.OneTo(endshape[2])] for i in Base.OneTo(endshape[1])]
    cubemap = reduce(vcat,permutedims.(collect.(cubemap)))
    faces = reshape([ !all(isequal(' '), cubemap[i]) for i in eachindex(cubemap)], endshape)
    @debug faces
    exit()

    # Determine the map cube net topology...
    Iterators.partition()
